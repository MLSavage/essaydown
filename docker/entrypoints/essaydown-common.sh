...
review_prompt() {
  ...
  cat <<EOF
You are the independent reviewer "$who" for Essay Down phase $phase, review attempt $attempt. Your worktree at /snapshot is checked out read-only at implementation_sha $impl; the phase base SHA is $base (both recorded in $dir/). Read docs/PRD.md (§7 gate row for phase $phase, §8 tasks for phase $phase, §9), docs/RUNNER-SPEC.md, docs/lessons.md, then \`git diff $base...$impl\`, /logs/ci/${verifier}h/accepted/ (this attempt's verifier; verify artifact digests against accepted.json) and the phase's other /logs/ci/<id>/accepted/, /logs/human/<id>/accepted.json and /logs/tasks/<id>/ directories. Try to falsify every gate criterion. Reviewers never edit code; the only writable persistent path is /report/ (= $dir/$who/). Write your report to /report/report.md using docs/reviews/TEMPLATE.md exactly (gate table with evidence, test counts, coverage delta, at most 20 findings with severity blocker|should-fix|nit, file:line and a concrete fix, three riskiest things, class-level lessons), then write /report/status.json as {"reviewer":"$who","phase":"$phase","attempt":"$attempt","implementation_sha":"$impl","verdict":"PASS"|"FAIL","blockers":<n>,"should_fix":<n>,"nits":<n>}. No praise, no summary of the code.
EOF
}
