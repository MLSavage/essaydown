...
exec node --test --test-concurrency=1 "$HERE/validator.test.mjs" "$HERE/util.test.mjs" "$HERE/conformance.test.mjs" "$HERE/journal.test.mjs" "$HERE/summary.test.mjs" "$HERE/check.test.mjs" "$@"
