...
done_=$(get done true); echoDone=$(get echoDone false); journal=$(get journal true); commit=$(get commit true); failSuite=$(get failSuite false); failCandidate=$(get failCandidate false)
mkdir -p "$(dirname "$log")"
cd "$wt"
...
{ echo "{\"type\":\"system\",\"task\":\"$id\"}"; [ "$echoDone" = "true" ] && echo "{\"type\":\"user\",\"message\":{\"content\":[{\"type\":\"tool_result\",\"content\":\"lessons: print <promise>DONE $id</promise>\"}]}}"; if [ "$done_" = "true" ]; then echo "{\"type\":\"result\",\"result\":\"<promise>DONE $id</promise>\"}"; else echo "{\"type\":\"result\",\"result\":\"not finished\"}"; fi; } > "$log"
