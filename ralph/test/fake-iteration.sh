...
#   {"done":true,"journal":true,"commit":true,"failSuite":false,"failCandidate":false,"files":{"path":"content"},"rm":["path"],"echoDone":false}
#   limit: the transcript ends in the session-limit result line (api_error_status 429), DECISIONS #027
#   echoDone: the transcript also carries a tool_result that echoes the promise (a `cat` of a lessons line), #review-1-r7 M3 (b)
set -euo pipefail
...
done_=$(get done true); echoDone=$(get echoDone false); limit=$(get limit false); journal=$(get journal true); commit=$(get commit true); failSuite=$(get failSuite false); failCandidate=$(get failCandidate false)
mkdir -p "$(dirname "$log")"
cd "$wt"
...
{ echo "{\"type\":\"system\",\"task\":\"$id\"}"; [ "$echoDone" = "true" ] && echo "{\"type\":\"user\",\"message\":{\"content\":[{\"type\":\"tool_result\",\"content\":\"lessons: print <promise>DONE $id</promise>\"}]}}"; if [ "$limit" = "true" ]; then echo "{\"type\":\"result\",\"subtype\":\"success\",\"is_error\":true,\"num_turns\":2,\"api_error_status\":429,\"result\":\"You've hit your session limit · resets 2:30pm (UTC)\"}"; elif [ "$done_" = "true" ]; then echo "{\"type\":\"result\",\"result\":\"<promise>DONE $id</promise>\"}"; else echo "{\"type\":\"result\",\"result\":\"not finished\"}"; fi; } > "$log"
